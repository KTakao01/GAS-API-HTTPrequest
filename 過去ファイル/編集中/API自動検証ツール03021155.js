...
function mainAuthPost() {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var sht = ss.getSheetByName('test');
...
function mainPost(authId, authMethod, authAuth, sht,authKeyRowNumber, authValueRowNumber) {
  ...
  for (var k = 0; k < idArray.length; k = k + 2) {
    ...
    if (id.indexOf("認証") == -1 && method == "POST") {
      ...
      //認証必要APIについて
      if (auth == authId) {
        referenceAuthPost(id, authId, method, authMethod, auth, authAuth, sht, keyRowNumber, valueRowNumber, authKeyRowNumber, authValueRowNumber);
        outputPostToWritten(id, method, sht, keyRowNumber, valueRowNumber);
...
function mainNoPost(authId, authMethod, authAuth, sht,authKeyRowNumber, authValueRowNumber) {
  ...
  for (var k = 0; k < idArray.length; k = k + 2) {
    ...
    if (id.indexOf("認証") == -1 && method != "POST") {
      ...
      //認証APIのレスポンスに基づいてリクエストを投げる必要があるAPIのリクエスト処理
      //ここから記述する（まだ未完成0301
...
function referenceAuthPost(id, authId, method, authMethod, auth, authAuth, sht, keyRowNumber, valueRowNumber, authKeyRowNumber, authValueRowNumber) {

  //参照するAPIを選択する（改修予定）//"必要"部分を変数化する必要がある参照

  //keyRowNumber,valueRowNumberとは別軸に書き込み先のkey-valueを取得する必要あり。//二重ループ。

  var referValueAccessToken = sht.getRange(authValueRowNumber, 11).getValue();
  var referValueUserId = sht.getRange(authValueRowNumber, 12).getValue();

  var referKeyAccessTokenPre = sht.getRange(authKeyRowNumber, 11).getValue();
  var referKeyUserIdPre = sht.getRange(authKeyRowNumber, 12).getValue();

  var referKeyAccessToken = referKeyAccessTokenPre.substring(indexOf("access_token"), referKeyAccessTokenPre.length());
  var referKeyUserId = referKeyUserIdPre.substring(indexOf("user_id"), referKeyUserIdPre.length());

  if (auth == authId) {
    if (referValueAccessToken != "" || referValueUserId != "") {
      //参照APIのパラメータkey取得
      var rangeParam = sht.getRange(keyRowNumber, 13, 1, sht.getLastColumn() - 12)
      const refKeys2array = rangeParam.getValues()
      var refKey1array = refKeys2array.flat()

      console.log(refKey1array);
      for (l = 0; l < refKey1array.length; l++) {
        if (refKey1array[l] == referKeyUserId) {
          sht.getRange(valueRowNumber, 12).setValue(refeerValueUserId);
        }
        else if (refKey1array[l] == referKeyAccessToken) {
          sht.getRange(valueRowNumber, 11).setValue(referValueAccessToken);
        }
