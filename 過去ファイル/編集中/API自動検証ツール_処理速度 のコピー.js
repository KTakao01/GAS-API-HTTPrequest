...
function referenceAuthtest2() {
  ...
  for (var m = startH; m < numHI; m = m + 2) {
    ...
    for (var n = startI; n < numHI; n = n + 2) {
      ...
      var getSheet = sht.getRange(referredKeyRowNumber, 1, 1, sht.getLastColumn()).getValues().flat()
      var startJ = 0

      //var numJ = referredKey1array.length
      var numJ = getSheet.length
        ...
        if (referredAuth == referId) {
          ...
          for (count = startJ; count < referredKey1array.length; count++) {
            ...
            if (referValueUserId != "" && referredKey1array[count] == referKeyUserId) {
              console.log("user_idのkeyです。書き込み処理を行う予定です。");
              dataId[n][count] = referValueUserId
              console.log(dataId)
            }
            else if (referValueAccessToken != "" && referredKey1array[count] == referKeyAccessToken) {
              console.log("access_tokenのkeyです。書き込み処理を行う予定です。");
              dataToken[n][count] = referValueAccessToken
              console.log(dataToken)
            }
            else {
              console.log("user_idおよびaccess_tokenのkeyではありません。書き込み処理は行いません。")
              dataId[n][count] = sht.getRange(n, count).getValue()
              dataToken[n][count] = sht.getRange(n, count).getValue()
    ...
    console.log("参照APIパラメーターのkeyのうちuser_idとaccess_tokenについて、以下の通り処理を行います。")
    console.log("user_idのの書き込み処理を行います。");
   // sht.getRange(startI + 1, startJ + 1, numHI, numJ).setValues(dataId);
    console.log("access_tokenの書き込み処理を行います。");
    //sht.getRange(startI + 1, startJ + 1, numHI, numJ).setValues(dataToken);
