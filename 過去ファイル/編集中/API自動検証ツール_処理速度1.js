...
function referenceAuthtest5() {
  ...
  for (var m = startH; m < numHI; m = m + 2) {
    ...
    lineArray[referKeyRowNumber - 1] = [];
    lineArray[referValueRowNumber - 1] = [];
      ...
      if (referredId.indexOf("認証") == -1 && referId.indexOf("認証") != -1) {
        ...
        if (referredAuth == referId) {
          ...
          for (col = startJ; col < numJ; col++) {
            ...
            lineArray[referKeyRowNumber - 1][col] = sht.getRange(referKeyRowNumber, col + 1).getValue();
            lineArray[referValueRowNumber - 1][col] = sht.getRange(referValueRowNumber, col + 1).getValue();
            if (lineArray[referKeyRowNumber - 1][col] == "" || lineArray[referValueRowNumber - 1][col] == "") {
              lineArray[referKeyRowNumber - 1][col] = null
              lineArray[referValueRowNumber - 1][col] = null
