...
function main() {
  ...
  for (var k = 0; k < idArray.length; k++) {
    ...
    if (method == "GET") {
      ...
      //書き込む処理はmain()で行う。sendXX()はリクエストとログ書き出し。
      const { getMessage, getStatusCode } = sendGetRequest(sht, rowNumber , method);
      ...
      const { postMessage, postStatusCode } = sendPostRequest(sht, rowNumber , method);
      sht.getRange(rowNumber, 5).setValue(postStatusCode);

    }

    else if (method == "PUT") {
      const { putMessage, putStatusCode } = sendPutRequest(sht, rowNumber , method);
      sht.getRange(rowNumber, 5).setValue(putStatusCode);

    }

    else if (method == "DELETE") {
      const { deleteMessage, deleteStatusCode } = sendDeleteRequest(sht, rowNumber ,method);
