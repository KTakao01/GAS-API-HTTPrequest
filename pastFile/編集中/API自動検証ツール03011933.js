...
function mainPost() {
  ...
  for (var k = 0; k < idArray.length; k = k + 2) {
    ...
    if (id != "認証" && method == "POST") {
      ...
      //認証必要APIについて
      if (auth == "必要") {
        referenceAuthPost(id, method,auth, sht, keyRowNumber, valueRowNumber);
        outputPostToWritten(id, method, sht, keyRowNumber, valueRowNumber);
...
function outputAuthPostToWritten(id, method, sht, keyRowNumber, valueRowNumber) {

  if (method == "POST" && id == "認証") {
    //認証APIのレスポンスを認証APIのvalueに書き出し
    const { authPostString, authPostMessage, authPostStatusCode, authAccessToken, authUserId } = sendAuthPostRequest(sht, keyRowNumber, valueRowNumber, method);
    sht.getRange(valueRowNumber, 6).setValue(authPostString);
    sht.getRange(valueRowNumber, 7).setValue(authPostMessage);
    sht.getRange(valueRowNumber, 8).setValue(authPostStatusCode);

    sht.getRange(valueRowNumber, 11).setValue(authAccessToken);
    sht.getRange(valueRowNumber, 12).setValue(authUserId);
...
function referenceAuthPost(id, method,auth, sht, keyRowNumber, valueRowNumber){
...
function mainAuthPost() {
  ...
  for (var k = 0; k < idArray.length; k = k + 2) {
    ...
    //認証と非認証APIの切り分け
    if (id == "認証" && method == "POST") {
      outputAuthPostToWritten(id, method, sht, keyRowNumber, valueRowNumber);
...
function sendAuthPostRequest(sht, keyRowNumber, valueRowNumber, method) {
  //パラメータのkeyを2次元配列として取得。1次元配列へ変換して配列要素抽出
  const range = sht.getRange(keyRowNumber, 13, 1, sht.getLastColumn() - 12)
  ...
  const rangeParam = sht.getRange(valueRowNumber, 13, 1, sht.getLastColumn() - 12)
  ...
  var authPostApi = sht.getRange(valueRowNumber, 3).getValue();
  var authPostCombi = sht.getRange(valueRowNumber, 5).getValue();
  ...
  //リクエストとレスポンスのログ排出
  console.log(method + " " + authPostApi + "についてaccess_tokenとuser_idの組み合わせが" + authPostCombi + "の時、リクエストとレスポンスは以下の通りです。\n※エンドポイント、JSONペイロード、レスポンスメッセージ、ステータスコードの順に記載")
  console.log(urlPost);
