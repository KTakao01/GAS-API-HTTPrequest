  ...
  function referenceAuthtest7() {
    ...
    for (var m = startH; m < numHI; m = m + 2) {
      ...
      for (var n = 0; n < referIdArray.length; n += 2) {
        ...
        lineArray[referredKeyRowNumber - 1][4] = sht.getRange(referredKeyRowNumber, 4).getValues();
        ...
        lineArray[referredKeyRowNumber - 1][1] = sht.getRange(referredKeyRowNumber, 4).getValues();
        ...
        for (col = startJ; col < numJ; col++) {
          ...
          //if (referredId.indexOf("認証") == -1 && referId.indexOf("認証") != -1) {
            if (lineArray[referredKeyRowNumber - 1][1].indexOf("認証") == -1 && referId.indexOf("認証") != -1) {
            ...
            console.log("認証API(No." + referId + ")を参照するAPI(No." + lineArray[referredKeyRowNumber - 1][1] + ")のパラメータを取得しています。");
            //指定した認証APIを参照する
            if (lineArray[referredKeyRowNumber - 1][4] == referId) {
              //参照APIのパラメータkey取得
              ...
              console.log("参照API(No." + lineArray[referredKeyRowNumber - 1][1] + ")が指定した認証API(No." + lineArray[referredKeyRowNumber - 1][4] + ")と一致しませんでした。処理いたしません。")
            }
  
          }
  
          else {
            console.log("対象API(No." + referId + ")は認証APIではないか、対象API(No." + lineArray[referredKeyRowNumber - 1][1] + ")は参照APIでないため、処理いたしません。")
          }
    ...
    //}
    console.log(lineArray[5])
    console.log(lineArray[5][10])
  
    console.log(lineArray[5][11])
